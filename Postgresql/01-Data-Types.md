# PostgreSQL
PostgreSQL is an advanced, open-source object-relational database system. It supports both SQL (relational) and JSON (non-relational).
...
## The psql CLI - Essential Commands
```bash
...
Schemas are logical containers that organize database objects. Other DBs doesn't have this
```sql
...
GRANT SELECT, INSERT ON ALL TABLES IN SCHEMA sales TO sales_team;
...
## Integer Types
PostgreSQL has several integer types with different storage requirements and ranges.
```sql
...
## Numeric Types
The NUMERIC type provides exact precision for financial calculations and scientific data.
```sql
-- Numeric examples
...
Real and double precision types for approximate numeric values.
```sql
...
## Storing Money
PostgreSQL provides a specialized MONEY type, but NUMERIC is often preferred for international applications.
```sql
-- Money type example
...
## NaNs and Infinity
PostgreSQL handles special numeric values like NaN (Not a Number) and infinity.
```sql
...
## Casting Types
Type conversion between different data types using explicit and implicit casting.
```sql
-- Casting examples
...
## Character Types
String data types with different characteristics and use cases.
```sql
...
## Domain Types
Create reusable custom data types with built-in constraints and default values.
```sql
...
## Binary Data
Store and manipulate binary data using BYTEA type.
```sql
-- Binary data table
...
## UUIDs
Universally Unique Identifiers for distributed systems and primary keys.
```sql
...
## Boolean Types
Boolean logic and three-valued logic (TRUE, FALSE, NULL).
```sql
-- Boolean examples
...
## Enums
Custom enumerated types for controlled vocabularies.
```sql
-- Create enum types
...
## Timestamps
Handle date and time data with precision and timezone awareness.
```sql
-- Timestamp types
...
## Dates and Times
Comprehensive date and time manipulation for scheduling and reporting.
```sql
-- Date and time types
